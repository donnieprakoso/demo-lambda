...
class CdkStack(Stack):
    def __init__(self, scope: Construct, id: str, stack_prefix: str,
        ...
        fnLambda_monolith = _lambda.Function(
            self,
            "{}-function-saveData".format(stack_prefix),
            code=_lambda.AssetCode("../lambda-functions/monolith"),
            handler="app.handler",
            timeout=core.Duration.seconds(60),
            role=lambda_role,
            tracing=_lambda.Tracing.ACTIVE,
            runtime=_lambda.Runtime.PYTHON_3_8)
        fnLambda_monolith.add_environment("TABLE_NAME", ddb_table.table_name)
        ...
        int_monolith= _ag.LambdaIntegration(fnLambda_monolith)

        # URL example: api.example.com/data/
        res_data = api.root.add_resource('data')
        res_data.add_method('POST', int_monolith)
        res_data.add_method('GET', int_monolith)

        # URL example: api.example.com/data/12345
        res_data_id = res_data.add_resource('{id}')
        res_data_id.add_method('GET', int_monolith)
        res_data_id.add_method('DELETE', int_monolith)
